...
def sanitise_word(word: str) -> str:
    """Removes the non-alpha characters from the beginning and end of the input string, in a recursive manner."""

    if len(word) == 0:
        return word

    result = f"{word[0] if word[0].isalpha() else ""}{word[1:-1]}{word[-1] if word[-1].isalpha() else ""}"
    return result if word == result else sanitise_word(result)


def main():
    sanitised_text = [sanitise_word(word) for word in TEXT.lower().split()]
    result = dict([(w, sanitised_text.count(w)) for w in sanitised_text if len(w) > 0])

    [print(f"{key} -> {value}") for key, value in sorted(result.items(), key=lambda e: e[1], reverse=True)]
